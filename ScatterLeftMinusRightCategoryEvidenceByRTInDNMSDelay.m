...
%for overallsubject = [12 15 16 18 20 25]
for overallsubject = [ 28 29 33 34 35] 
...
%for this one, we'll be looking at left vs. right
    faceCount1 = 1;
    for block = 1:5
        for trial = 1:12
            ...
            %category 3 = left scence = global learning list 4
            %category 4 = right scence = global learning list 2
            if mod(parsedData(block, trial).Category, 2) == 1 
                SameCat = parsedData(block, trial).leftScene + parsedData(block, trial).leftFace;
                OtherCat = parsedData(block, trial).rightScene + parsedData(block, trial).rightFace;
                faceCount1 = faceCount1 + 1;
            elseif mod(parsedData(block, trial).Category, 2) == 0
                SameCat = parsedData(block, trial).rightScene + parsedData(block, trial).rightFace;
                OtherCat = parsedData(block, trial).leftFace + parsedData(block, trial).leftFace;
                faceCount1 = faceCount1 + 1;
            end
...
figure(subjectCount)
hold on
title(['Subject ' num2str(overallsubject) ': Left - Right Category Evidence during DNMS Delay * RT'])
...
xlabel('Left - Other Category Evidence')
ylabel('RT')
hold off
